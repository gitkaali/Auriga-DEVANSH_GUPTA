...
type User = { id: number; email: string; name: string; role: 'ADMIN' | 'AGENT'; isActive?: boolean }
...
type Audit = { action: string; previousValue: string | null; newValue: string | null; actorType: string; occurredAt: string; actor: { name: string } | null }
type Dashboard = { total: number; open: number; overdue: number; assigned: number; byPriority: Array<{ priority: string; _count: { _all: number } }>; byStatus: Array<{ status: string; _count: { _all: number } }>; recentlyEscalated: Array<{ action: string; ticket: { id: number; title: string }; occurredAt: string; previousValue: string | null; newValue: string | null }> }
...
function App() {
  ...
  const [filters, setFilters] = useState({ search: '', priority: '', status: '', overdue: '', assigneeId: '' })
  ...
  const updateTicket = async (id: number, field: 'status' | 'priority' | 'promisedResponseAt', value: string) => { await api(`/tickets/${id}`, { method: 'PATCH', body: JSON.stringify({ [field]: value }) }); await load() }
  ...
  const openTicket = async (id: number) => { try { const [ticket, audit] = await Promise.all([api<Ticket>(`/tickets/${id}`), api<Audit[]>(`/tickets/${id}/audit`)]); setSelectedTicket(ticket); setSelectedAudit(audit) } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Unable to load ticket') } }
  const deleteTicket = async (id: number) => { if (!window.confirm('Delete this ticket permanently?')) return; await api(`/tickets/${id}`, { method: 'DELETE' }); setSelectedTicket(null); await load() }
  const createUser = async (event: FormEvent) => { event.preventDefault(); await api('/users', { method: 'POST', body: JSON.stringify(newUser) }); setNewUser({ name: '', email: '', password: '', role: 'AGENT' }); const refreshed = await api<User[]>('/users'); setUsers(refreshed) }
  const toggleUser = async (managedUser: User) => { await api(`/users/${managedUser.id}`, { method: 'PATCH', body: JSON.stringify({ isActive: managedUser.isActive === false }) }); setUsers(await api<User[]>('/users')) }
  ...
  return <div className="app-shell"><header className="topbar"><div><p className="eyebrow">AURIGA / HELP DESK</p><h1>Response control room</h1></div><div className="user-menu"><span>{user.name}<small>{user.role}</small></span>{user.role === 'ADMIN' && <button className="quiet-button" onClick={() => setShowUsers(true)}>Manage users</button>}<button className="quiet-button" onClick={logout}>Sign out</button></div></header>
    <main className="content"><section className="metrics">{[['TOTAL', dashboard?.total ?? 0], ['OPEN', dashboard?.open ?? 0], ['OVERDUE', dashboard?.overdue ?? 0], ['MY QUEUE', dashboard?.assigned ?? 0]].map(([label, value]) => <div className="metric" key={label as string}><span>{label}</span><strong>{value}</strong></div>)}</section>
      {dashboard && <section className="breakdowns"><div><p className="eyebrow">PRIORITY MIX</p>{dashboard.byPriority.map((item) => <span key={item.priority}>{item.priority} <b>{item._count._all}</b></span>)}</div><div><p className="eyebrow">STATUS MIX</p>{dashboard.byStatus.map((item) => <span key={item.status}>{item.status} <b>{item._count._all}</b></span>)}</div></section>}
    ...
    </main>{showCreate && <div className="modal-backdrop"><form className="modal" onSubmit={createTicket}><div className="modal-head"><div><p className="eyebrow">NEW WORK</p><h2>Create ticket</h2></div><button type="button" className="close" onClick={() => setShowCreate(false)}>×</button></div><label>Customer<input required value={newTicket.customerName} onChange={(e) => setNewTicket({ ...newTicket, customerName: e.target.value })} /></label><label>Title<input required value={newTicket.title} onChange={(e) => setNewTicket({ ...newTicket, title: e.target.value })} /></label><label>Description<textarea required rows={4} value={newTicket.description} onChange={(e) => setNewTicket({ ...newTicket, description: e.target.value })} /></label><label>Response due<input required type="datetime-local" value={newTicket.promisedResponseAt} onChange={(e) => setNewTicket({ ...newTicket, promisedResponseAt: e.target.value })} /></label><button className="primary" type="submit">Create ticket</button></form></div>}{selectedTicket && <div className="modal-backdrop"><section className="modal"><div className="modal-head"><div><p className="eyebrow">TICKET #{selectedTicket.id}</p><h2>{selectedTicket.title}</h2></div><button type="button" className="close" onClick={() => setSelectedTicket(null)}>×</button></div><p>{selectedTicket.description}</p><label>Response due<input type="datetime-local" defaultValue={new Date(selectedTicket.promisedResponseAt).toISOString().slice(0, 16)} onChange={(e) => void updateTicket(selectedTicket.id, 'promisedResponseAt', new Date(e.target.value).toISOString())} /></label>{user.role === 'ADMIN' && <button className="danger-button" onClick={() => void deleteTicket(selectedTicket.id)}>Delete ticket</button>}<div className="audit-list"><p className="eyebrow">AUDIT HISTORY</p>{selectedAudit.map((item) => <p key={`${item.occurredAt}-${item.action}`}><b>{item.action}</b><span>{item.actor?.name ?? item.actorType} · {new Date(item.occurredAt).toLocaleString()}</span></p>)}</div></section></div>}{showUsers && <div className="modal-backdrop"><section className="modal"><div className="modal-head"><div><p className="eyebrow">ADMINISTRATION</p><h2>Manage users</h2></div><button type="button" className="close" onClick={() => setShowUsers(false)}>×</button></div><div className="user-list">{users.map((managedUser) => <p key={managedUser.id}><span><b>{managedUser.name}</b><small>{managedUser.email} · {managedUser.role}</small></span><button className="quiet-button" disabled={managedUser.id === user.id} onClick={() => void toggleUser(managedUser)}>{managedUser.isActive === false ? 'Activate' : 'Deactivate'}</button></p>)}</div><form onSubmit={createUser}><label>Name<input required value={newUser.name} onChange={(e) => setNewUser({ ...newUser, name: e.target.value })} /></label><label>Email<input required type="email" value={newUser.email} onChange={(e) => setNewUser({ ...newUser, email: e.target.value })} /></label><label>Temporary password<input required minLength={12} type="password" value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} /></label><label>Role<select value={newUser.role} onChange={(e) => setNewUser({ ...newUser, role: e.target.value })}><option>AGENT</option><option>ADMIN</option></select></label><button className="primary" type="submit">Create user</button></form></section></div>}
  </div>
}
